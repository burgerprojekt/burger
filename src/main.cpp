...
#if defined(NDEBUG)
# error "BurgerKing cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6902e9dbd2e8edabc1eb685f3fc774df8830029e7b4a810cb7efc7822848e650");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BurgerKing: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BurgerKing Signed Message:\n";
...
{
    // BurgerKing: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 3 * 60; // BurgerKing: 3 minutes
static const int64 nTargetSpacing = 3 * 30; // BurgerKing: 1.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BurgerKing: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BurgerKing: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6902e9dbd2e8edabc1eb685f3fc774df8830029e7b4a810cb7efc7822848e650");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Eat BURGERS! 24.08.17";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042111710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1503597705;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3009785;
        ...
        assert(block.hashMerkleRoot == uint256("0xa3e459b162407453d54449062c4612782d0167af9e4b22e02e72d7254d9170ef"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xaa, 0xdb }; // BurgerKing: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BurgerKingMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BurgerKingMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BurgerKingMiner(CWallet *pwallet)
{
    printf("BurgerKingMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("burgerking-miner");
    ...
    try { loop {
        ...
        printf("Running BurgerKingMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BurgerKingMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BurgerKingMiner, pwallet));
}
